...
(ert-deftest acapella-a2a-normalize-jsonrpc-error-hints ()
  "Normalization adds friendly message and hint for known A2A codes (A2A §8.2)."
  ;; Use -32002 TaskNotCancelableError per spec
  (let* ((err '(("code" . -32002) ("message" . "cannot cancel now"))))
    (let* ((norm (acapella-a2a--normalize-jsonrpc-error err))
      ...
      (should (string-match-p "Task cannot be canceled" msg))
      (should (string-match-p "cannot cancel now" msg))
      (should (stringp hint)))))
